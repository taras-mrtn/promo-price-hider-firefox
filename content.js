...
function hidePromoPrice() {
  ...
  // "Що таке Картка Rozetka?" button (contains promo price + red card image)
  document.querySelectorAll('button[title*="Картка Rozetka"]').forEach(button => {
  ...
  // Rozetka AI chat bot button
  if (hideAiButton) {
    document.querySelectorAll('rz-chat-bot-button-assist').forEach(button => {
      if (hideElement(button)) hiddenCount++;
    });
  }
